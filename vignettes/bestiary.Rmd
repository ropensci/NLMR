...
# create landscapes
set.seed(5)
percolation <- nlm_percolation(100,100,0.4)
random <- nlm_random(100,100)
distagrad <- nlm_distancegradient(100, 100, c(10,15,10,15))
edgy <- nlm_edgegradient(100,100, 90)
planary <- nlm_planargradient(100,100)
randomRecClustery <- nlm_randomrectangularcluster(100, 100, 8, 16, rescale = FALSE)
randomElementy <- nlm_randomelement(100,100, 90)
randomClustery <- nlm_randomcluster(100,100, p = 0.5, neighbourhood = 4)
randomMPD <- nlm_mpd(nCol = 100, nRow = 100, roughness = 0.7)
gaussian <- nlm_gaussianfield(nCol = 100, nRow = 100, 35)
gaussian_linear <- nlm_gaussianfield(nCol = 100, nRow = 100, 35, direction = "linear")
...
randomElementy <- nlm_randomelement(100,100, 90)
randomClustery <- nlm_randomcluster(100,100, p = 0.5, neighbourhood = 4)
gaussian <- nlm_gaussianfield(nCol = 100, nRow = 100, 5)
gaussian_linear <- nlm_gaussianfield(nCol = 100, nRow = 100, 5, direction = "linear")
...
# Plot the rasters as facets
nlmr_merges <- ggplot(nlm_df, aes(x=x, y=y)) +
  geom_raster(aes(fill=value), alpha=0.8) +
  facet_wrap( ~ Type, ncol = 3, scales = "fixed") +
  ggplot2::coord_equal() +
  ggplot2::labs(x = "Easting",
