...
__NOTE__: The version on CRAN is 
...
Landscape models simulated by nlmr are returned as raster objects, hence they can
be processed as usual, e.g. visualized:
```{r echo = FALSE}
nlm_raster <- raster::ratify(nlm_raster) 
rat <- raster::levels(nlm_raster)[[1]]
rat$legend <- c('Forest', 'Grassland', 'Crops')
levels(nlm_raster) <- rat
```
