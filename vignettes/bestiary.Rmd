...
gaussian <- nlm_gaussianfield(ncol = 100, nrow = 100, autocorr_range = 35, resolution = 10)
fBm <- nlm_fBm(ncol = 50, nrow = 50, fractal_dim = 1, resolution = 20)
...
wheys <- nlm_wheys(p = c(0.33, 0.33, 0.33), s = c(4, 8, 16), q = c(0.22, 0.22, 0.22), ext = 1000)
...
wheys_spdf          <- as(wheys, "SpatialPixelsDataFrame")
...
wheys_df <- as.data.frame(wheys_spdf)
wheys_df[,4] <- "Wheyed curdling"
...
merge_1 <- util_merge(gaussian, randomRecClustery)
...
merge_1_df[,4] <- "Gaussian + Random Rectangular Cluster"
